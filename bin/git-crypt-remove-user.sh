...
msg="(git-crypt-remove-user.sh $1)"
...
git commit -a -m "Remove encrypted files $msg"
rm -rf .git-crypt
git commit -a -m "Remove git-crypt $msg"
rm -rf .git/git-crypt
...
git commit -a -m "New encrypted files $msg"
popd

git crypt lock
git pull $TMPDIR/$BASENAME $BRANCH
